...
const g = {
  ...
  P = (n) => [g.prefix, n, g.suffix].filter((e) => e && e.length > 0).join("-"),
  A = (n) => {
    for (const e of Object.keys(g)) n(e);
  },
  L = {
    updateDetails: (n) => {
      A((e) => {
    ...
    getGoogleAnalyticsName: (n) => n || P(g.googleAnalytics),
    getPrecacheName: (n) => n || P(g.precache),
    getPrefix: () => g.prefix,
    getRuntimeName: (n) => n || P(g.runtime),
    getSuffix: () => g.suffix,
  };
function E(n, e) {
  const t = e();
...
function M(n) {
  if (!n) throw new u("add-to-cache-list-unexpected-type", { entry: n });
  if (typeof n == "string") {
    const r = new URL(n, location.href);
    return { cacheKey: r.href, url: r.href };
  }
  const { revision: e, url: t } = n;
  if (!t) throw new u("add-to-cache-list-unexpected-type", { entry: n });
  if (!e) {
  ...
  return s.searchParams.set(F, e), { cacheKey: s.href, url: a.href };
}
class q {
  constructor() {
    (this.updatedURLs = []),
...
async function B(n, e) {
  ...
  const s = n.clone(),
    ...
    o = H() ? s.body : await s.blob();
  return new Response(o, r);
}
const $ = (n) =>
  new URL(String(n), location.href).href.replace(
...
class X {
  ...
  async fetch(e) {
    ...
    try {
      ...
      for (const c of this.iterateCallbacks("fetchDidSucceed"))
        o = await c({ event: t, request: r, response: o });
      return o;
    } catch (o) {
      throw (
        (a &&
  ...
  async cachePut(e, t) {
    ...
    if (!t) throw new u("cache-put-with-no-response", { url: $(a.url) });
    ...
    const { cacheName: o, matchOptions: c } = this._strategy,
      ...
      d = h ? await z(l, a.clone(), ["__WB_REVISION__"], c) : null;
    try {
      await l.put(a, h ? r.clone() : r);
    } catch (f) {
      if (f instanceof Error)
        throw (f.name === "QuotaExceededError" && (await J()), f);
    }
...
class Z {
  ...
  handleAll(e) {
    ...
    const t = e.event,
      ...
      r = new X(this, { event: t, request: s, params: a }),
      o = this._getResponse(r, s, t),
      c = this._awaitComplete(o, r, s, t);
    return [o, c];
  }
    ...
    try {
      ...
      if (r instanceof Error) {
        for (const o of e.iterateCallbacks("handlerDidError"))
          if (((a = await o({ error: r, event: s, request: t })), a)) break;
      }
      if (!a) throw r;
    }
  ...
  async _handleFetch(e, t) {
    ...
    if (this._fallbackToNetwork) {
      const r = a.integrity,
        ...
        c = !o || o === r;
      (s = await t.fetch(
        new Request(e, { integrity: e.mode !== "no-cors" ? o || r : void 0 }),
      )),
        r &&
          c &&
          e.mode !== "no-cors" &&
  ...
  _useDefaultCacheabilityPluginIfNeeded() {
    ...
    for (const [s, a] of this.plugins.entries())
      a !== p.copyRedirectedCacheableResponsesPlugin &&
        (a === p.defaultPrecacheCacheabilityPlugin && (e = s),
        a.cacheWillUpdate && t++);
    t === 0
...
p.copyRedirectedCacheableResponsesPlugin = {
  async cacheWillUpdate({ response: n }) {
    return n.redirected ? await B(n) : n;
  },
};
class Y {
  constructor({
    ...
    for (const s of e) {
      ...
      if (typeof s != "string" && s.integrity) {
        if (
          ...
          throw new u("add-to-cache-list-conflicting-integrities", { url: r });
        this._cacheKeysToIntegrities.set(a, s.integrity);
      }
      if (
        (this._urlsToCacheKeys.set(r, a),
        this._urlsToCacheModes.set(r, o),
        t.length > 0)
      ) {
  ...
  install(e) {
    return E(e, async () => {
      ...
      for (const [r, o] of this._urlsToCacheKeys) {
        const c = this._cacheKeysToIntegrities.get(o),
          l = this._urlsToCacheModes.get(r),
          h = new Request(r, {
            integrity: c,
            cache: l,
  ...
  activate(e) {
    return E(e, async () => {
      const t = await self.caches.open(this.strategy.cacheName),
        ...
        r = [];
      for (const o of s) a.has(o.url) || (await t.delete(o), r.push(o.url));
...
const x = "GET",
  b = (n) => (n && typeof n == "object" ? n : { handle: n });
class w {
  constructor(e, t, s = x) {
...
class te {
  ...
  handleRequest({ request: e, event: t }) {
    ...
    const a = s.origin === location.origin,
      { params: r, route: o } = this.findMatchingRoute({
        event: t,
        request: e,
        sameOrigin: a,
        url: s,
    ...
    if (
      (!c &&
        ...
        (c = this._defaultHandlerMap.get(l)),
      !c)
    )
      return;
    let h;
  ...
  findMatchingRoute({ url: e, sameOrigin: t, request: s, event: a }) {
    ...
    for (const o of r) {
      let c;
      const l = o.match({ url: e, sameOrigin: t, request: s, event: a });
      if (l)
        return (
          (c = l),
...
let R;
const se = () => (
  R || ((R = new te()), R.addFetchListener(), R.addCacheListener()), R
);
function T(n, e, t) {
  let s;
  if (typeof n == "string") {
    const r = new URL(n, location.href),
      o = ({ url: c }) => c.href === r.href;
    s = new w(o, e, t);
  } else if (n instanceof RegExp) s = new ee(n, e, t);
...
const ce = "-precache-",
  ie = async (n, e = ce) => {
    const s = (await self.caches.keys()).filter(
      (a) => a.includes(e) && a.includes(self.registration.scope) && a !== n,
    );
    return await Promise.all(s.map((a) => self.caches.delete(a))), s;
  };
function le() {
  self.addEventListener("activate", (n) => {
    const e = L.getPrecacheName();
    n.waitUntil(ie(e).then((t) => {}));
  });
}
function he(n) {
  return N().createHandlerBoundToURL(n);
}
function ue(n) {
  N().precache(n);
}
...
const ge = "webank-db",
  ...
  we = () =>
    new Promise((n, e) => {
      ...
      const t = indexedDB.open(ge, pe);
      (t.onerror = (s) => {
        console.error("[SW] Database error:", s.target.error),
          e(new Error("Database error"));
      }),
        (t.onsuccess = (s) => {
          console.log("[SW] Database opened successfully"), n(s.target.result);
        }),
        (t.onupgradeneeded = (s) => {
          console.log("[SW] Database upgrade needed");
          const a = s.target.result;
          a.objectStoreNames.contains(U) ||
            a.createObjectStore(U, { keyPath: "id" });
        });
    });
self.addEventListener("message", (n) => {
  n.data &&
    n.data.type === "SKIP_WAITING" &&
    (console.log("[SW] Skipping waiting"), self.skipWaiting());
});
self.addEventListener("install", (n) => {
  console.log("[SW] Installing service worker"),
    n.waitUntil(self.skipWaiting());
});
self.addEventListener("activate", (n) => {
  console.log("[SW] Activating service worker"),
    n.waitUntil(Promise.all([self.clients.claim(), le()]));
});
fe([
  {
    revision: "2adb6736f5c719d377d8bfd226572a4d",
    url: "android-chrome-144x144.png",
  },
  {
    revision: "023a421985eb1a1374381e58ecbede77",
    url: "android-chrome-192x192.png",
  },
  {
    revision: "6c0ad7bc6af938f167e6d3601892a628",
    url: "android-chrome-512x512.png",
  },
  { revision: "51946c6c39e3a2bfdda2d91b1590bdaa", url: "apple-touch-icon.png" },
  { revision: null, url: "assets/index-CmgX4t9h.js" },
  { revision: null, url: "assets/index-noM_esVg.css" },
  { revision: null, url: "assets/projectEnvVariables-CKUWVdfg.js" },
  { revision: "537eb96f1edb9502a4ac203ea7b3415a", url: "back-id.png" },
  { revision: "537eb96f1edb9502a4ac203ea7b3415a", url: "BackId.png" },
  { revision: "71dcfd191507c31dc79efe3341dfa3b9", url: "favicon copy.svg" },
  { revision: "7bfbc531bdae98c8dcf7303f11205b94", url: "favicon.ico" },
  { revision: "71dcfd191507c31dc79efe3341dfa3b9", url: "favicon.svg" },
  { revision: "fea0a40b683b611731904b67a4e63467", url: "front-id.png" },
  { revision: "a0db21e9b59e25c6c83f58cda37cda27", url: "index.html" },
  { revision: "f0402b67b6ce880f65666bb49e841696", url: "react.svg" },
  {
    revision: "edd56bc92721de008aceec78f9bf2f1d",
    url: "Screenshot_dashboard.png",
  },
  { revision: "66ad29d2cb0c981205f7bae6ba42d317", url: "Screenshot_otp.png" },
  {
    revision: "4649c1492f3fe09d3375bebe42fe4727",
    url: "Screenshot_register.png",
  },
  { revision: "5b605622c85f66c193ca2c161ae17761", url: "selfie-id.png" },
  { revision: "945c850826c6ebf53827953eda2d5d9e", url: "Tax.png" },
  { revision: "f7e0e91da27ddd030734fadb604c2a5c", url: "Webank.png" },
  {
    revision: "2adb6736f5c719d377d8bfd226572a4d",
    url: "android-chrome-144x144.png",
  },
  ...
  { revision: "b85bab8d30c7302ce6124ce16a0a6593", url: "manifest.webmanifest" },
]);
let ye;
T(new de(he("index.html"), { allowlist: ye }));
T(
  ({ url: n }) => {
    const e = n.pathname === "/share-handler";
    return (
      console.log(`[SW] Intercepted request: ${n.href}, isShareHandler: ${e}`),
      e
    );
  },
  async ({ request: n }) => {
    ...
    try {
      ...
      if (
        (console.log("[SW] Form data:", {
          ...
          url: a,
          fileCount: r.length,
          files: r.map((i) => ({ name: i.name, type: i.type, size: i.size })),
        }),
        !r.length)
      )
        return (
          console.error("[SW] No files received"),
          new Response("No files shared", { status: 400 })
        );
      const o = 100 * 1024,
        c = r.filter((i) => i.size > o);
      if (c.length > 0)
        return (
          console.error(
          ...
          new Response(
            `File size exceeds 100KB limit: ${c.map((i) => i.name).join(", ")}`,
            { status: 400 },
          )
        );
      const l = await Promise.all(
          r.map(async (i) => {
            console.log(
              `[SW] Processing file: ${i.name} (${i.type}, ${i.size} bytes)`,
            );
            try {
              const y = await me(i);
              return { name: i.name, type: i.type, size: i.size, base64: y };
            } catch (y) {
              throw (
                (console.error(`[SW] Failed to process file ${i.name}:`, y),
                new Error(`Failed to process file ${i.name}`))
              );
            }
          }),
        ),
        h = { title: t, text: s, url: a, files: l },
        d = JSON.stringify(h).length;
      if ((console.log("[SW] Shared content size:", d), d > 10 * 1024 * 1024))
        return (
          console.error("[SW] Content too large for IndexedDB:", d),
          new Response("File too large (max 10MB)", { status: 400 })
        );
      const O = (await we()).transaction(U, "readwrite").objectStore(U);
      console.log("[SW] Storing data in IndexedDB"),
        await new Promise((i, y) => {
          const W = O.put({ id: "sharedContent", ...h });
          (W.onsuccess = () => i()),
            (W.onerror = () => y(new Error("Failed to store shared content")));
        }),
        console.log("[SW] Data stored successfully");
      const k = await self.clients.matchAll({
        type: "window",
        includeUncontrolled: !0,
      });
      if ((console.log("[SW] Found clients:", k.length), k.length === 0)) {
        console.warn("[SW] No clients found, opening new window");
        const i = await self.clients.openWindow("/share-handler");
        console.log("[SW] New window opened:", i ? "Success" : "Failed");
      } else
        for (const i of k)
          console.log("[SW] Sending RELOAD_CONTENT to client:", i.id),
            i.postMessage({ type: "RELOAD_CONTENT", data: h });
      return (
        console.log("[SW] Redirecting to /share-handler"),
        Response.redirect("/share-handler", 303)
      );
    } catch (e) {
      return (
        console.error("[SW] Error handling share:", e),
        new Response("Error processing share: ", { status: 500 })
      );
    }
  },
  "POST",
...
async function me(n) {
  return new Promise((e, t) => {
    ...
    (s.onload = () => {
      console.log("[SW] File conversion completed"), e(s.result);
    }),
      (s.onerror = () => {
        console.error("[SW] File conversion failed"),
          t(new Error("Failed to read file as base64"));
      }),
      s.readAsDataURL(n);
  });
}
