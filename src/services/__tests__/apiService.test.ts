...
import { sendOTP } from '../keyManagement/apiService'; // Adjust the import path
...
describe('sendOTP', () => {
  ...
  it('should send OTP successfully with valid inputs', async () => {
    ...
    mock.onPost('http://localhost:8080/api/registration').reply(200, { message: 'OTP sent' });
  ...
  it('should throw an error when API responds with an error', async () => {
    ...
    await expect(sendOTP(fullPhoneNumber, jwtToken)).rejects.toThrow('Failed to send OTP');
  });
