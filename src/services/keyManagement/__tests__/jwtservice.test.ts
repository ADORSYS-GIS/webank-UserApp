...
describe("JWT Generation", () => {
  it("should generate a valid JWT with a hash of the payload and a valid signature", async () => {
    ...
    const expectedHash = CryptoJS.SHA256(JSON.stringify(data)).toString(CryptoJS.enc.Hex);
    ...
    const { payload } = await jose.jwtVerify(jwt, await jose.importJWK(publicKeyJWK, "ES256"), {
      algorithms: ["ES256"],
    });
