...
export type ButtonProps<As extends React.ElementType> = React.ComponentPropsWithoutRef<As> & {
  as?: As;
  block?: boolean;
  fullWidth?: boolean;
  color?: Color;
...
const filledButtonColors: Record<Color, string> = {
  ...
  secondary: 'bg-secondary text-white hover:bg-secondary-content focus:ring-secondary',
...
const outlinedButtonColors: Record<Color, string> = {
  primary: 'border-2 border-primary-light text-primary-content hover:bg-primary-light',
  secondary: 'border-2 border-secondary-light text-secondary-content hover:bg-secondary-light',
  neutral: 'border-2 border-neutral-light text-neutral-content hover:bg-neutral-light',
  danger: 'border-2 border-danger-light text-danger-content hover:bg-danger-light',
  success: 'border-2 border-success-light text-success-content hover:bg-success-light',
};
...
export function Button<As extends React.ElementType = 'button'>({
  ...
  return (
    <Component
      {...rest}
      className={twMerge('w-full py-4 px-6 rounded-lg transition-all duration-200 font-medium transform focus:outline-none focus:ring-2 focus:ring-opacity-50', [
        variant === 'outlined' && [outlinedButtonColors[color], 'backdrop-blur-md'],
        variant === 'soft' && [softButtonColors[color], 'backdrop-blur-md'],
        variant === 'filled' && [filledButtonColors[color], 'hover:-translate-y-1 shadow-md hover:shadow-lg backdrop-blur-md'],
        block && 'block',
        fullWidth && 'w-full',
        rest.className,
      ])}
